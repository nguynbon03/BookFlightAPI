...
from app.schemas.user import UserCreate, UserUpdate,UserUpdateNotAdmin
...
def update_user(session: Session, user: User, data:UserUpdateNotAdmin ) -> User:
    ...
    session.refresh(user)
    return user
def update_superuser(session: Session, user: User, data:UserUpdate ) -> User:
    update_data = data.dict(exclude_unset=True)  # chỉ lấy field được gửi lên
